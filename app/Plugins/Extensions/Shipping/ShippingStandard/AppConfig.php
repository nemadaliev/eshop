...
class AppConfig extends ConfigDefault
{
    ...
    public function __construct()
    {
        ...
        $this->version = '1.0';
        $this->auth = 'Naruto';
        $this->link = 'https://eshop.utunity.com';
    }
    ...
    public function config()
    {
        return view($this->pathPlugin.'::ShippingStandard')->with(
            [
                ...
                'datas' => ShippingStandardModel::all(),
            ]);
    }
    ...
    public function getData()
    {
        ...
        $shippings = ShippingStandardModel::all()->where('status', 1);

        foreach ($shippings as $index => $shipping) {

            if ( $subtotal >= $shipping->min_total && $subtotal <= $shipping->max_total ) {
                $arrData = [
                    'title' => $this->title,
                    'code' => $this->configKey,
                    'image' => $this->image,
                    'permission' => self::ALLOW,
                    'value' => $shipping['is_free'] === 1 ? 0 : $shipping['fee'],
                    'version' => $this->version,
                    'auth' => $this->auth,
                    'link' => $this->link,
                ];
